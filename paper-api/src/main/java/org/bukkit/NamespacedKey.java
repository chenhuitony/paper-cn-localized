...
public final class NamespacedKey implements Key, com.destroystokyo.paper.Namespaced {
    ...
    public NamespacedKey(@NotNull String namespace, @NotNull String key) {
        Preconditions.checkArgument(namespace != null, "命名空间不能为 null");
    ...
    private void validate() {
        Preconditions.checkArgument(this.namespace.length() + 1 + this.key.length() <= Short.MAX_VALUE, "NamespacedKey 长度必须小于 32768 个字符");
    ...
    private static void checkError(String pattern, String name, String value, OptionalInt index) {
        index.ifPresent(indexValue -> {
            if (indexValue == -1) {
                throw new IllegalArgumentException(String.format("'%s' 不是 %s 的有效值", value, name));
            } else {
                char character = value.charAt(indexValue);
                throw new IllegalArgumentException(String.format("%s '%s' 的第 %d 个索引处存在非 %s 字符（'%s'，字节：%s）", pattern, name, value, indexValue, character, Arrays.toString(String.valueOf(character).getBytes(StandardCharsets.UTF_8))));
            }
        });
    }
    ...
    public static NamespacedKey fromString(@NotNull String string, @Nullable Plugin defaultNamespace) {
        Preconditions.checkArgument(string != null, "输入字符串不能为 null");
