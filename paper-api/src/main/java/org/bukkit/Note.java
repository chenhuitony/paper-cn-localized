...
public class Note {
    ...
    public enum Tone {
        ...
        public boolean isSharped(byte id) {
            if (id == getId(false)) {
                ...
                throw new IllegalArgumentException("id 与音调不匹配。");
            }
        }
    ...
    public Note(int note) {
        Preconditions.checkArgument(note >= 0 && note <= 24, "音符值必须在 0 到 24 之间。");
    ...
    public Note(int octave, @NotNull Tone tone, boolean sharped) {
        ...
        if (octave < 0 || octave > 2 || (octave == 2 && !(tone == Tone.F && sharped))) {
            throw new IllegalArgumentException("音调和八度必须在 F#0 到 F#2 之间");
        }
    ...
    public static Note flat(int octave, @NotNull Tone tone) {
        Preconditions.checkArgument(octave != 2, "平坦世界的八度不能为 2");
    ...
    public static Note natural(int octave, @NotNull Tone tone) {
        Preconditions.checkArgument(octave != 2, "自然世界的八度不能为 2");
    ...
    @NotNull
    public Note sharped() {
        Preconditions.checkArgument(note < 24, "此音符无法升调，因为它已是已知最高音符！");
    ...
    public Note flattened() {
        Preconditions.checkArgument(note > 0, "此音符无法降调，因为它已是已知最低音符！");
        return new Note(note - 1);
    }
