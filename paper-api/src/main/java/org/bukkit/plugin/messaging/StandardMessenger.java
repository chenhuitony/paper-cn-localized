...
public class StandardMessenger implements Messenger {
        ...
        synchronized (incomingLock) {
            ...
            if (registrations == null) {
                ...
                if (registrations.contains(registration)) {
                    throw new IllegalArgumentException("此注册已存在");
                }
            }
    ...
    public boolean isRegistrationValid(@NotNull PluginMessageListenerRegistration registration) {
        if (registration == null) {
            throw new IllegalArgumentException("注册项不能为 null");
        }
    ...
    public void dispatchIncomingMessage(@NotNull Player source, @NotNull String channel, byte @NotNull [] message) {
        if (source == null) {
            throw new IllegalArgumentException("玩家来源不能为 null");
        }
        if (message == null) {
        ...
        for (PluginMessageListenerRegistration registration : registrations) {
            try {
                ...
                registration.getPlugin().getLogger().log(Level.WARNING,
                    String.format("插件 %s 在处理插件消息时生成异常",
    ...
    public static String validateAndCorrectChannel(@NotNull String channel) {
        if (channel == null) {
            throw new IllegalArgumentException("通道不能为 null");
        }
        ...
        if (channel.indexOf(':') == -1) {
            throw new IllegalArgumentException("通道必须包含 : 分隔符（尝试使用 " + shortened(channel) + ")");
        }
        if (!channel.toLowerCase(Locale.ROOT).equals(channel)) {
            // TODO: use NamespacedKey validation here
            throw new IllegalArgumentException("通道必须全部为小写（尝试使用 " + shortened(channel) + ")");
        }
        return channel;
    }
    ...
    public static void validatePluginMessage(@NotNull Messenger messenger, @NotNull Plugin source, @NotNull String channel, byte @NotNull [] message) {
        if (messenger == null) {
            throw new IllegalArgumentException("Messenger 不能为 null");
        }
        if (source == null) {
            throw new IllegalArgumentException("插件来源不能为 null");
        }
        if (!source.isEnabled()) {
            throw new IllegalArgumentException("插件必须处于启用状态才能发送消息");
        }
        if (message == null) {
            throw new IllegalArgumentException("消息不能为 null");
        }
