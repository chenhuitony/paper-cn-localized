...
public final class PluginClassLoader extends URLClassLoader implements io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader { // Paper
    ...
    public PluginClassLoader(@Nullable final ClassLoader parent, @NotNull final PluginDescriptionFile description, @NotNull final File dataFolder, @NotNull final File file, @Nullable ClassLoader libraryLoader, JarFile jarFile, io.papermc.paper.plugin.provider.entrypoint.DependencyContext dependencyContext) throws IOException, InvalidPluginException, MalformedURLException { // Paper - use JarFile provided by SpigotPluginProvider
        ...
        try {
            ...
            throw new InvalidPluginException("找不到主类 `" + description.getMain() + "'", ex);
        }
        ...
        try {
            ...
            throw new InvalidPluginException("主类 `" + description.getMain() + "' 必须继承 JavaPlugin", ex);
        }
        ...
        try {
            ...
            throw new InvalidPluginException("主类 `" + description.getMain() + "' 必须有无参构造函数", ex);
        }
        ...
        try {
            ...
            throw new InvalidPluginException("主类 `" + description.getMain() + "' 的构造函数不可访问", ex);
        }
        ...
        try {
            ...
            throw new InvalidPluginException("主类 `" + description.getMain() + "' 不能是抽象类", ex);
        } catch (IllegalArgumentException ex) {
            throw new InvalidPluginException("无法调用主类 `" + description.getMain() + "' 的构造函数", ex);
        } catch (ExceptionInInitializerError | InvocationTargetException ex) {
            throw new InvalidPluginException("初始化主类时发生异常 `" + description.getMain() + "'", ex);
        }
    }
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        if (result == null) {
            ...
            try {
                ...
                throw new IllegalStateException("用于以下插件的类加载器：" + plugin.getName() + " 抛出了一个 zip 文件错误。", zipFileClosed);
            }
                ...
                if (dot != -1) {
                    ...
                    if (getPackage(pkgName) == null) {
                        try {
                            ...
                            if (getPackage(pkgName) == null) {
                                throw new IllegalStateException("找不到包 " + pkgName);
                            }
                        }
                    }
                }
    ...
    public synchronized void initialize(@NotNull JavaPlugin javaPlugin) { // Paper
        Preconditions.checkArgument(javaPlugin != null, "待初始化的插件不能为 null");
        Preconditions.checkArgument(javaPlugin.getClass().getClassLoader() == this, "无法在此类加载器之外初始化插件");
        if (this.plugin != null || this.pluginInit != null) {
            throw new IllegalArgumentException("插件已初始化！", pluginState);
        }

        pluginState = new IllegalStateException("尚未初始化");
