...
// Paper end
    ...
    public LibraryLoader(@NotNull Logger logger) {
        ...
        session.setTransferListener(new AbstractTransferListener() {
            ...
            public void transferStarted(@NotNull TransferEvent event) {
                logger.log(Level.INFO, "正在下载 {0}", event.getResource().getRepositoryUrl() + event.getResource().getResourceName());
            }
        });
    ...
    public ClassLoader createLoader(@NotNull PluginDescriptionFile desc, java.util.@Nullable List<java.nio.file.Path> paperLibraryPaths) {
        ...
        logger.log(Level.INFO, "[{0}] 正在加载 {1} 个库…请稍候", new Object[]
            {
        ...
        if (!dependencies.isEmpty()) {
            ...
            try
            {
                ...
                throw new RuntimeException("解析库时出错", ex);
            }
        } else {
        ...
        for (java.nio.file.Path path : jarPaths) {
            ...
            jarFiles.add(url);
            logger.log(Level.INFO, "[{0}] 已加载库 {1}", new Object[]
                {
