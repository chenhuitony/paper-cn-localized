...
public final class JavaPluginLoader implements PluginLoader {
    ...
    public JavaPluginLoader(@NotNull Server instance) {
        Preconditions.checkArgument(instance != null, "服务端不能为 null");
        server = instance;
        ...
        try {
            ...
            server.getLogger().warning("无法初始化 LibraryLoader（缺少依赖？）");
        }
    ...
    public Plugin loadPlugin(@NotNull final File file) throws InvalidPluginException {
        ...
        if (dataFolder.equals(oldDataFolder)) {
            ...
            if (!oldDataFolder.renameTo(dataFolder)) {
                throw new InvalidPluginException("无法重命名旧数据文件夹：`" + oldDataFolder + "' 到：`" + dataFolder + "'");
            }
            server.getLogger().log(Level.INFO, String.format(
                "加载 %s（%s）时将数据文件夹 `%s' 重命名为 `%s'",
                description.getFullName(),
                file,
                oldDataFolder,
                dataFolder
        ...
        for (final String pluginName : description.getDepend()) {
            ...
            if (current == null) {
                throw new UnknownDependencyException("未知的依赖 " + pluginName + "。请下载并安装 " + pluginName + "，才能运行此插件。");
            }
        }
    ...
    public PluginDescriptionFile getPluginDescription(@NotNull File file) throws InvalidDescriptionException {
        ...
        try {
            ...
            if (entry == null) {
                throw new InvalidDescriptionException(new FileNotFoundException("Jar 包中不包含 plugin.yml"));
            }
    ...
    public Map<Class<? extends Event>, Set<RegisteredListener>> createRegisteredListeners(@NotNull Listener listener, @NotNull final Plugin plugin) {
        ...
        try {
            ...
            plugin.getLogger().severe("Plugin " + plugin.getDescription().getFullName() + " 注册事件失败，目标为 " + listener.getClass() + " because " + e.getMessage() + " 不存在。");
            return ret;
        }
        ...
        for (final Method method : methods) {
            ...
            if (method.getParameterTypes().length != 1 || !Event.class.isAssignableFrom(checkClass = method.getParameterTypes()[0])) {
                plugin.getLogger().severe(plugin.getDescription().getFullName() + " 尝试注册无效的 EventHandler 方法签名 \"" + method.toGenericString() + "\" in " + listener.getClass());
                continue;
            }
    ...
    public void enablePlugin(@NotNull final Plugin plugin) {
        Preconditions.checkArgument(plugin instanceof JavaPlugin, "插件未与此 PluginLoader 关联");
        ...
        if (!plugin.isEnabled()) {
            ...
            if (!loaders.contains(pluginLoader)) {
                ...
                server.getLogger().log(Level.WARNING, "启用了使用未注册 PluginClassLoader 的插件 " + plugin.getDescription().getFullName());
            }

            try {
                jPlugin.setEnabled(true);
            } catch (Throwable ex) {
                server.getLogger().log(Level.SEVERE, "启用以下内容时发生错误：" + plugin.getDescription().getFullName() + "（它是否已是最新版本？）", ex);
            }
    ...
    public void disablePlugin(@NotNull Plugin plugin) {
        ...
        if (plugin.isEnabled()) {
            String message = String.format("正在禁用 %s", plugin.getDescription().getFullName());
            ...
            try {
                ...
                server.getLogger().log(Level.SEVERE, "禁用以下内容时发生错误：" + plugin.getDescription().getFullName() + "（它是否已是最新版本？）", ex);
            }
