...
public abstract class JavaPlugin extends PluginBase {
    ...
    public JavaPlugin() {
        if (this.getClass().getClassLoader() instanceof io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader configuredPluginClassLoader) {
            ...
            throw new IllegalStateException("JavaPlugin 必须由有效的类加载器创建。");
        }
    }
    ...
    protected JavaPlugin(final JavaPluginLoader loader, final PluginDescriptionFile description, final File dataFolder, final File file) {
        ...
        if (classLoader instanceof PluginClassLoader) {
            throw new IllegalStateException("无法在运行时使用初始化构造函数");
        }
    ...
    public void saveConfig() {
        try {
            ...
            logger.log(Level.SEVERE, "无法将配置保存到 " + configFile, ex);
        }
    }
    ...
    public void saveResource(String resourcePath, boolean replace) {
        if (resourcePath == null || resourcePath.equals("")) {
            throw new IllegalArgumentException("ResourcePath 不能为 null 或空");
        }

        resourcePath = resourcePath.replace('\\', '/');
        InputStream in = getResource(resourcePath);
        if (in == null) {
            throw new IllegalArgumentException("内嵌资源 '" + resourcePath + "' 无法在以下位置找到：" + file);
        }
        ...
        try {
            if (!outFile.exists() || replace) {
                ...
                logger.log(Level.WARNING, "无法保存 " + outFile.getName() + " to " + outFile + " because " + outFile.getName() + " 已存在。");
            }
        } catch (IOException ex) {
            logger.log(Level.SEVERE, "无法保存 " + outFile.getName() + " to " + outFile, ex);
        }
    }
    ...
    public @Nullable PluginCommand getCommand(String name) {
        if (this.isBeingEnabled && !(pluginMeta instanceof PluginDescriptionFile)) {
            throw new UnsupportedOperationException("""
                你正在启动期间对 Paper 插件调用 JavaPlugin#getCommand：
                你很可能是想获取一个在 paper-plugin.yml 中定义的命令。
                Paper 插件不支持基于 YAML 的命令声明！
                你可以使用 JavaPlugin#registerCommand 在 Paper 插件中定义命令。
                请查阅文档了解如何在 Paper 插件中定义命令：https://docs.papermc.io/paper/dev/getting-started/paper-plugins#commands
                """);
        }
    ...
    public static <T extends JavaPlugin> T getPlugin(Class<T> clazz) {
        Preconditions.checkArgument(clazz != null, "null 类不能拥有插件");
        if (!JavaPlugin.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(clazz + " 不是 " + JavaPlugin.class);
        }
        ...
        if (!(cl instanceof io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader configuredPluginClassLoader)) {
            throw new IllegalArgumentException(clazz + " 不是由以下类初始化的：" + io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader.class);
        }
        JavaPlugin plugin = configuredPluginClassLoader.getPlugin();
        if (plugin == null) {
            throw new IllegalStateException("无法为以下内容获取插件：" + clazz + " 从静态初始化块中");
        }
    ...
    public static JavaPlugin getProvidingPlugin(Class<?> clazz) {
        ...
        final ClassLoader cl = clazz.getClassLoader();
        if (!(cl instanceof io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader configuredPluginClassLoader)) {
            throw new IllegalArgumentException(clazz + " 不是由以下类提供的：" + io.papermc.paper.plugin.provider.classloader.ConfiguredPluginClassLoader.class);
        }
        JavaPlugin plugin = configuredPluginClassLoader.getPlugin();
        if (plugin == null) {
            throw new IllegalStateException("无法为以下内容获取插件：" + clazz + " 从静态初始化块中");
