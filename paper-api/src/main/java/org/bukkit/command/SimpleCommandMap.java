...
public class SimpleCommandMap implements CommandMap {
    ...
    public boolean dispatch(@NotNull CommandSender sender, @NotNull String commandLine) throws CommandException {
        ...
        try {
            ...
            //target.timings.stopTiming(); // Spigot // Paper
            String msg = "执行 ' 时发生未处理异常" + commandLine + "'，位于 " + target;
            server.getPluginManager().callEvent(new com.destroystokyo.paper.event.server.ServerExceptionEvent(new com.destroystokyo.paper.exception.ServerCommandException(ex, target, sender, args))); // Paper
    ...
    public List<String> tabComplete(@NotNull CommandSender sender, @NotNull String cmdLine, @Nullable Location location) {
        ...
        Preconditions.checkArgument(cmdLine != null, "命令行不能为 null");
        ...
        try {
            ...
            String msg = "执行 ' 的 Tab 补全器时发生未处理异常" + cmdLine + "'，位于 " + target;
    ...
    public void registerServerAliases() {
        ...
        for (Map.Entry<String, String[]> entry : values.entrySet()) {
            ...
            if (alias.contains(" ")) {
                server.getLogger().warning("无法注册别名 " + alias + "，因为它包含非法字符");
                continue;
            }
            ...
            if (bad.length() > 0) {
                server.getLogger().warning("无法注册别名 " + alias + "，因为它包含不存在的命令: " + bad);
                continue;
            }
