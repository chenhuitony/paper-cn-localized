    ...
    public static List<Command> parse(@NotNull Plugin plugin) {
        ...
        for (Entry<String, Map<String, Object>> entry : map.entrySet()) {
            if (entry.getKey().contains(":")) {
                Bukkit.getServer().getLogger().severe("无法加载命令 " + entry.getKey() + "，用于插件 " + plugin.getName() + ": 非法字符");
                continue;
            }
                ...
                if (aliases instanceof List) {
                    for (Object o : (List<?>) aliases) {
                        if (o.toString().contains(":")) {
                            Bukkit.getServer().getLogger().severe("无法加载别名 " + o.toString() + "，用于插件 " + plugin.getName() + ": 非法字符");
                            continue;
                        }
                    ...
                    if (aliases.toString().contains(":")) {
                        Bukkit.getServer().getLogger().severe("无法加载别名 " + aliases.toString() + "，用于插件 " + plugin.getName() + ": 非法字符");
                    } else {
