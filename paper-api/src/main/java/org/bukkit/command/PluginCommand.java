...
public final class PluginCommand extends Command implements PluginIdentifiableCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        if (!owningPlugin.isEnabled()) {
            throw new CommandException("无法执行命令 '" + commandLabel + "'，位于插件 " + owningPlugin.getDescription().getFullName() + " - 插件已禁用。");
        }
        ...
        try {
            ...
            throw new CommandException("执行命令 ' 时发生未处理异常" + commandLabel + "'，位于插件 " + owningPlugin.getDescription().getFullName(), ex);
        }
    ...
    public java.util.List<String> tabComplete(@NotNull CommandSender sender, @NotNull String alias, @NotNull String[] args) throws CommandException, IllegalArgumentException {
        Preconditions.checkArgument(sender != null, "发送者不能为 null");
        Preconditions.checkArgument(args != null, "参数不能为 null");
        Preconditions.checkArgument(alias != null, "别名不能为 null");
        ...
        try {
            ...
            message.append("命令 '/' 的 Tab 补全期间发生未处理异常").append(alias).append(' ');
            for (String arg : args) {
                message.append(arg).append(' ');
            }
            message.deleteCharAt(message.length() - 1).append("'，位于插件 ").append(owningPlugin.getDescription().getFullName());
