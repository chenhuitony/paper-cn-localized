...
public class HelpCommand extends BukkitCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String currentAlias, @NotNull String[] args) {
        ...
        if (topic == null || !topic.canSee(sender)) {
            sender.sendMessage(Component.text("没有对应帮助：" + command, NamedTextColor.RED));
            return true;
        }
    ...
    protected HelpTopic findPossibleMatches(@NotNull String searchString) {
        ...
        if (possibleMatches.size() > 0) {
            return new IndexHelpTopic("Search", null, null, possibleMatches, "搜索：" + searchString);
        } else {
            return null;
        }
    }
