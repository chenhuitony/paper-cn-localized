...
public final class StructureType implements Keyed {
    ...
    private StructureType(@NotNull String name, @Nullable MapCursor.Type mapIcon) {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(name), "结构名称不能为空");
    ...
    private static <T extends StructureType> T register(@NotNull T type) {
        Preconditions.checkNotNull(type, "无法注册 null 的结构类型。");
        Preconditions.checkArgument(!structureTypeMap.containsKey(type.getName()), "无法重复注册相同的结构类型。%s", type.getName());
