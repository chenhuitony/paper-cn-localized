...
public interface Criteria {
    ...
    public static Criteria statistic(@NotNull Statistic statistic, @NotNull Material material) {
        Preconditions.checkArgument(statistic != null, "statistic 不能为 null");
        Preconditions.checkArgument(material != null, "material 不能为 null");

        Type type = statistic.getType();
        Preconditions.checkArgument(type == Type.BLOCK || type == Type.ITEM, "统计类型必须为 BLOCK 或 ITEM，实际为 %s", type);
        Preconditions.checkArgument(type != Type.BLOCK || material.isBlock(), "统计类型为 BLOCK，但得到了非方块 Material，%s", material);
        Preconditions.checkArgument(type != Type.ITEM || material.isItem(), "统计类型为 ITEM，但得到了非物品 Material，%s", material);
    ...
    public static Criteria statistic(@NotNull Statistic statistic, @NotNull EntityType entityType) {
        ...
        Preconditions.checkArgument(statistic.getType() == Type.ENTITY, "统计类型必须为 ENTITY，实际为 %s", statistic.getType());
