...
public class PermissibleBase implements Permissible {
    ...
    public void setOp(boolean value) {
        if (opable == null) {
            throw new UnsupportedOperationException("由于未设置 ServerOperator，无法更改 op 值");
        } else {
            opable.setOp(value);
        }
    }
    ...
    public boolean isPermissionSet(@NotNull String name) {
        if (name == null) {
            throw new IllegalArgumentException("权限名称不能为 null");
        }
    ...
    public boolean isPermissionSet(@NotNull Permission perm) {
        if (perm == null) {
            throw new IllegalArgumentException("权限不能为 null");
        }
    ...
    public synchronized PermissionAttachment addAttachment(@NotNull Plugin plugin, @NotNull String name, boolean value) { // Paper - synchronized
        if (name == null) {
            ...
            throw new IllegalArgumentException("Plugin " + plugin.getDescription().getFullName() + " 已禁用");
        }
    ...
    public synchronized void removeAttachment(@NotNull PermissionAttachment attachment) { // Paper - synchronized
        if (attachment == null) {
            throw new IllegalArgumentException("附件不能为 null");
        }
        ...
        if (attachments.remove(attachment)) {
            ...
            throw new IllegalArgumentException("给定的附件不是以下 Permissible 对象的一部分：" + parent);
        }
    }
    ...
    public synchronized PermissionAttachment addAttachment(@NotNull Plugin plugin, int ticks) { // Paper - synchronized
        ...
        if (Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new RemoveAttachmentRunnable(result), ticks) == -1) {
            Bukkit.getServer().getLogger().log(Level.WARNING, "无法为以下对象添加 PermissionAttachment：" + parent + "，用于插件 " + plugin.getDescription().getFullName() + ": 调度器返回 -1");
            result.remove();
            return null;
        } else {
            return result;
