...
public class SpawnRule implements Cloneable, ConfigurationSerializable {
    ...
    public SpawnRule(int minBlockLight, int maxBlockLight, int minSkyLight, int maxSkyLight) {
        Preconditions.checkArgument(minBlockLight <= maxBlockLight, "minBlockLight 必须 <= maxBlockLight（%s <= %s）", minBlockLight, maxBlockLight);
        Preconditions.checkArgument(minSkyLight <= maxSkyLight, "minSkyLight 必须 <= maxSkyLight（%s <= %s）", minSkyLight, maxSkyLight);
        Preconditions.checkArgument(minBlockLight >= 0, "minBlockLight 必须 >= 0（给定 %s）", minBlockLight);
        Preconditions.checkArgument(maxBlockLight >= 0, "maxBlockLight 必须 >= 0（给定 %s）", maxBlockLight);
        Preconditions.checkArgument(minSkyLight >= 0, "minSkyLight 必须 >= 0（给定 %s）", minSkyLight);
        Preconditions.checkArgument(maxSkyLight >= 0, "maxSkyLight 必须 >= 0（给定 %s）", maxSkyLight);
