...
public interface RecipeChoice extends Predicate<ItemStack>, Cloneable {
    ...
    sealed class MaterialChoice implements RecipeChoice permits ItemTypeRecipeChoiceImpl {
        ...
        public MaterialChoice(Tag<Material> choices) {
            this(new ArrayList<>(java.util.Objects.requireNonNull(choices, "无法用 null 标签创建物品类型选择").getValues())); // Paper - delegate to list ctor to make sure all checks are called
        }

        public MaterialChoice(List<Material> choices) {
            Preconditions.checkArgument(choices != null, "choices");
            Preconditions.checkArgument(!choices.isEmpty(), "必须至少有一个选项");

            this.choices = new ArrayList<>(choices.size());

            for (Material choice : choices) {
                Preconditions.checkArgument(choice != null, "选择项不能为 null");

                if (choice.isLegacy()) {
                    choice = Bukkit.getUnsafe().fromLegacy(new MaterialData(choice, (byte) 0), true);
                }

                Preconditions.checkArgument(!choice.isAir(), "不能使用空/空气作为选择");
                Preconditions.checkArgument(choice.isItem(), "不能使用非物品选择 %s", choice); // Paper - validate material choice input to items
        ...
        public RecipeChoice validate(final boolean allowEmptyRecipes) {
            if (this.choices.stream().anyMatch(Material::isAir)) {
                throw new IllegalArgumentException("RecipeChoice.MaterialChoice 不能包含空气");
            }
            return this;
        }
        // Paper end - check valid ingredients
    ...
    final class ExactChoice implements RecipeChoice {
        ...
        public ExactChoice(List<ItemStack> choices) {
            ...
            for (ItemStack choice : choices) {
                Preconditions.checkArgument(choice != null, "选择项不能为 null");
                Preconditions.checkArgument(!choice.getType().isAir(), "不能使用空/空气作为选择");
            }
        ...
        public RecipeChoice validate(final boolean allowEmptyRecipes) {
            if (this.choices.stream().anyMatch(s -> s.getType().isAir())) {
                throw new IllegalArgumentException("RecipeChoice.ExactChoice 不能包含空气");
            }
            return this;
        }
        // Paper end - check valid ingredients
