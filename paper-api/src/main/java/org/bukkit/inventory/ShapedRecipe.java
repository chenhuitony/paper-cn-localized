...
public class ShapedRecipe extends CraftingRecipe {
    ...
    public ShapedRecipe(@NotNull ItemStack result) {
        ...
        new Throwable("警告：有插件正在使用已弃用的方法创建配方。这将导致你收到警告，提示'Tried to load unrecognized recipe: bukkit:<ID>'。请要求作者使用 NamespacedKey 为其配方指定静态键。").printStackTrace(); // Paper
    }
    ...
    public ShapedRecipe shape(@NotNull final String... shape) {
        Preconditions.checkArgument(shape != null, "必须提供形状");
        Preconditions.checkArgument(shape.length > 0 && shape.length < 4, "合成配方应为 1、2 或 3 行，而非 ", shape.length);

        int lastLen = -1;
        for (String row : shape) {
            Preconditions.checkArgument(row != null, "形状不能包含 null 行");
            Preconditions.checkArgument(row.length() > 0 && row.length() < 4, "合成行应为 1、2 或 3 个字符，而非 ", row.length());

            Preconditions.checkArgument(lastLen == -1 || lastLen == row.length(), "合成配方必须是矩形");
            lastLen = row.length();
        }
    ...
    public ShapedRecipe setIngredient(char key, @NotNull Material ingredient) {
        Preconditions.checkArgument(key != ' ', "配方形状中的空格表示无配料");
        Preconditions.checkArgument(this.ingredients.containsKey(key), "符号未出现在形状中：", key);
    ...
    public ShapedRecipe setIngredient(char key, @NotNull ItemStack item) {
        Preconditions.checkArgument(!item.getType().isAir(), "物品不能为空气"); // Paper
