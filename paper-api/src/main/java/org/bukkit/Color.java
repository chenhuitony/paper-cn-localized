...
public final class Color implements ConfigurationSerializable {
    ...
    public static Color fromRGB(int rgb) throws IllegalArgumentException {
        Preconditions.checkArgument((rgb >> 24) == 0, "其中包含多余数据：%s", rgb);
    ...
    public static Color fromBGR(int bgr) throws IllegalArgumentException {
        Preconditions.checkArgument((bgr >> 24) == 0, "其中包含多余数据：%s", bgr);
    ...
    private Color(int alpha, int red, int green, int blue) {
        Preconditions.checkArgument(alpha >= 0 && alpha <= 0xff, "Alpha[%s] 不在 0-255 之间", alpha);
        Preconditions.checkArgument(red >= 0 && red <= 0xff, "Red[%s] 不在 0-255 之间", red);
        Preconditions.checkArgument(green >= 0 && green <= 0xff, "绿色分量[%s]不在 0-255 之间", green);
        Preconditions.checkArgument(blue >= 0 && blue <= 0xff, "Blue[%s] 不在 0-255 之间", blue);
    ...
    public Color mixDyes(@NotNull DyeColor... colors) {
        Preconditions.checkArgument(colors != null && Arrays.stream(colors).allMatch(Objects::nonNull), "DyeColor 不能为 null，也不能包含 null 值");
    ...
    public Color mixColors(@NotNull Color... colors) {
        Preconditions.checkArgument(colors != null && Arrays.stream(colors).allMatch(Objects::nonNull), "颜色列表不能为 null");
    ...
    private static int asInt(@NotNull String string, @NotNull Map<String, Object> map, @Nullable Object defaultValue) {
        ...
        if (value == null) {
            throw new IllegalArgumentException(string + " 不在以下映射表中：" + map);
        }
        if (!(value instanceof Number)) {
            throw new IllegalArgumentException(string + '(' + value + ") 不是数字");
        }
