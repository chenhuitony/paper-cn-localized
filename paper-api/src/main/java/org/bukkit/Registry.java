...
public interface Registry<T extends Keyed> extends Iterable<T> {
    ...
    private static <A extends Keyed> Registry<A> legacyRegistryFor(final Class<A> clazz) {
        return Objects.requireNonNull(RegistryAccess.registryAccess().getRegistry(clazz), () -> "不存在对应的注册表：" + clazz.getSimpleName() + "。这是一个 Bug。");
    }
    ...
    default @Nullable T get(final TypedKey<T> typedKey) {
        Preconditions.checkArgument(typedKey != null, "typedKey 不能为 null");
    ...
    default T getOrThrow(final net.kyori.adventure.key.Key key) {
        ...
        if (value == null) {
            throw new NoSuchElementException("没有对应值：" + key + " in " + this);
        }
        return value;
    }
    ...
    default NamespacedKey getKeyOrThrow(final T value) {
        ...
        if (key == null) {
            throw new NoSuchElementException(value + " 没有键，位于 " + this);
        }
        return key;
    }
    ...
    default @Nullable T match(final String input) {
        Preconditions.checkArgument(input != null, "input 不能为 null");
    ...
    abstract class NotARegistry<A extends Keyed> implements Registry<A> {
        ...
        public boolean hasTag(final TagKey<A> key) {
            throw new UnsupportedOperationException("这不是真正的注册表，因此不支持标签");
        }
