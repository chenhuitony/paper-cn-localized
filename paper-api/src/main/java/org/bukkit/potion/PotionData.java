...
public final class PotionData {
    ...
    public PotionData(@NotNull PotionType type, boolean extended, boolean upgraded) {
        Preconditions.checkArgument(type != null, "药水类型不能为 null");
        Preconditions.checkArgument(!upgraded || type.isUpgradeable(), "药水类型不可升级");
        Preconditions.checkArgument(!extended || type.isExtendable(), "药水类型不可延长");
        Preconditions.checkArgument(!upgraded || !extended, "药水不能同时被延长和升级");
        Preconditions.checkArgument(!type.getKey().getKey().startsWith("strong_"), "强效药水类型不能直接使用，实际为 %s", type.getKey());
        Preconditions.checkArgument(!type.getKey().getKey().startsWith("long_"), "延长型药水类型不能直接使用，实际为 %s", type.getKey());
        this.type = type;
