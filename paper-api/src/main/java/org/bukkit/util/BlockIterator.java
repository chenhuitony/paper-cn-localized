...
public class BlockIterator implements Iterator<Block> {
    ...
    public BlockIterator(@NotNull World world, @NotNull Vector start, @NotNull Vector direction, double yOffset, int maxDistance) {
        Preconditions.checkArgument(world != null, "world 不能为 null");
        Preconditions.checkArgument(start != null, "start 不能为 null");
        Preconditions.checkArgument(direction != null, "direction 不能为 null");
        Preconditions.checkArgument(!direction.isZero(), "direction 必须至少有一个非零分量");
        ...
        if (!startBlockFound) {
            throw new IllegalStateException("BlockIterator 中缺少起始方块");
        }
    ...
    @Override
    public void remove() {
        throw new UnsupportedOperationException("[BlockIterator] 不支持移除方块");
    }
