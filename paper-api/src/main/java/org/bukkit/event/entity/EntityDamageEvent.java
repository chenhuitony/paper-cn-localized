...
public class EntityDamageEvent extends EntityEvent implements Cancellable {
    ...
    public EntityDamageEvent(@NotNull final Entity damagee, @NotNull final DamageCause cause, @NotNull final DamageSource damageSource, @NotNull final Map<DamageModifier, Double> modifiers, @NotNull final Map<DamageModifier, ? extends Function<? super Double, Double>> modifierFunctions) {
        super(damagee);
        Preconditions.checkArgument(modifiers.containsKey(DamageModifier.BASE), "缺少 BASE DamageModifier");
        Preconditions.checkArgument(!modifiers.containsKey(null), "不能包含 null 的伤害修正");
        Preconditions.checkArgument(modifiers.values().stream().allMatch(Objects::nonNull), "修正值不能为 null");
        Preconditions.checkArgument(modifiers.keySet().equals(modifierFunctions.keySet()), "每个 DamageModifier 都必须有对应的修饰函数");
        Preconditions.checkArgument(modifierFunctions.values().stream().allMatch(Objects::nonNull), "修正函数不能为 null");
    ...
    public double getOriginalDamage(@NotNull DamageModifier type) throws IllegalArgumentException {
        Preconditions.checkArgument(type != null, "不能包含 null 的伤害修正");
    ...
    public void setDamage(@NotNull DamageModifier type, double damage) throws IllegalArgumentException, UnsupportedOperationException {
        ...
        if (!this.modifiers.containsKey(type)) {
            throw new UnsupportedOperationException(type + " 不适用于 " + getEntity());
        }
