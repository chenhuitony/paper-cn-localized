...
public class ConfigurationSerialization {
    ...
    protected ConfigurationSerializable deserializeViaMethod(@NotNull Method method, @NotNull Map<String, ?> args) {
        try {
            ...
            if (result == null) {
                Logger.getLogger(ConfigurationSerialization.class.getName()).log(Level.SEVERE, "无法调用方法 '" + method.toString() + "' 的 " + clazz + " 进行反序列化：方法返回了 null");
            } else {
                return result;
            }
        } catch (Throwable ex) {
            Logger.getLogger(ConfigurationSerialization.class.getName()).log(
                    Level.SEVERE,
                    "无法调用方法 '" + method.toString() + "' 的 " + clazz + " 进行反序列化",
    ...
    protected ConfigurationSerializable deserializeViaCtor(@NotNull Constructor<? extends ConfigurationSerializable> ctor, @NotNull Map<String, ?> args) {
        try {
            ...
            Logger.getLogger(ConfigurationSerialization.class.getName()).log(
                    Level.SEVERE,
                    "无法调用构造函数 '" + ctor.toString() + "' 的 " + clazz + " 进行反序列化",
                    ex instanceof InvocationTargetException ? ex.getCause() : ex);
        }
    ...
    public static ConfigurationSerializable deserializeObject(@NotNull Map<String, ?> args) {
        ...
        if (args.containsKey(SERIALIZED_TYPE_KEY)) {
            try {
                ...
                if (alias == null) {
                    throw new IllegalArgumentException("别名不能为 null");
                }
                clazz = getClassByAlias(alias);
                if (clazz == null) {
                    throw new IllegalArgumentException("指定的类不存在（'" + alias + "')");
                }
            ...
            throw new IllegalArgumentException("参数不包含类型键（'" + SERIALIZED_TYPE_KEY + "')");
        }
