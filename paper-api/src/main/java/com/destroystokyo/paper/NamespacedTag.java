...
public final class NamespacedTag implements com.destroystokyo.paper.Namespaced {
    ...
    public NamespacedTag(@NotNull String namespace, @NotNull String key) {
        Preconditions.checkArgument(namespace != null && VALID_NAMESPACE.matcher(namespace).matches(), "命名空间无效。必须为 [a-z0-9._-]：%s", namespace);
        Preconditions.checkArgument(key != null && VALID_KEY.matcher(key).matches(), "键无效。必须为 [a-z0-9/._-]：%s", key);

        this.namespace = namespace;
        this.key = key;

        String string = toString();
        Preconditions.checkArgument(string.length() < 256, "NamespacedTag 长度必须小于 256 个字符", string);
    }
    ...
    public NamespacedTag(@NotNull Plugin plugin, @NotNull String key) {
        ...
        Preconditions.checkArgument(VALID_NAMESPACE.matcher(this.namespace).matches(), "命名空间无效。必须为 [a-z0-9._-]：%s", this.namespace);
        Preconditions.checkArgument(VALID_KEY.matcher(this.key).matches(), "键无效。必须为 [a-z0-9/._-]：%s", this.key);

        String string = toString();
        Preconditions.checkArgument(string.length() < 256, "NamespacedTag 长度必须小于 256 个字符（%s）", string);
    }
