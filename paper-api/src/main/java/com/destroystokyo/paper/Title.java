...
public final class Title {
    ...
    public Title(@Nullable BaseComponent[] title, @NotNull BaseComponent[] subtitle, int fadeIn, int stay, int fadeOut) {
        checkArgument(fadeIn >= 0, "fadeIn 为负值：%s", fadeIn);
        checkArgument(stay >= 0, "stay 为负值：%s", stay);
        checkArgument(fadeOut >= 0, "fadeOut 为负值：%s", fadeOut);
