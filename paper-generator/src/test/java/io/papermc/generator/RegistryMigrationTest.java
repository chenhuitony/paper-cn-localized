...
public class RegistryMigrationTest {
    ...
    public void testBuiltInToDataDriven() {
        ...
        Assertions.assertTrue(migratedRegistries.isEmpty(), () -> "一些注册表已变为数据驱动：%s".formatted(String.join(", ", migratedRegistries)));
    }
    ...
    public void testDataDrivenToBuiltIn() { // shouldn't really happen but just in case
        ...
        Assertions.assertTrue(migratedRegistries.isEmpty(), () -> "一些注册表已变为内置：%s".formatted(String.join(", ", migratedRegistries)));
    }
}
