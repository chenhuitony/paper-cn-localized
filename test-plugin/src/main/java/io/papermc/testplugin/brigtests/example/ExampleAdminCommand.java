...
public class ExampleAdminCommand {
    ...
    public static void register(JavaPlugin plugin, Commands commands) {
        final LiteralArgumentBuilder<CommandSourceStack> adminBuilder = Commands.literal("admin")
            .executes((ct) -> {
                ct.getSource().getSender().sendPlainMessage("根管理员");
                return 1;
            })
            .then(
                Commands.literal("tp")
            ...
            .then(
                Commands.literal("ice_cream").then(
                    Commands.argument("type", new IceCreamTypeArgument()).executes((context) -> {
                        ...
                        context.getSource().getSender().sendMessage(Component.text("你喜欢：" + argumentResponse));
                        return 1;
                    })
                )
            )
        ...
        commands.register(plugin.getPluginMeta(), adminBuilder.build(), "展示你所能做到一切的酷炫命令！", List.of("alias_for_admin_that_you_shouldnt_use", "a"));
