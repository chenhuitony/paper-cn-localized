...
public final class SparksFly {
    ...
    public SparksFly(final Server server) {
        ...
        this.logger.log(Level.INFO, "此服务器内置了 spark 性能分析器。如需更多信息，请访问 https://docs.papermc.io/paper/profiling");
            ...
            private Runnable catching(final Runnable runnable, final String type) {
                return () -> {
                    try {
                        ...
                        SparksFly.this.logger.log(Level.SEVERE, "执行" + type + " spark 任务时遇到异常", t);
                    }
                };
            }
    ...
    public void enableAfterPlugins(final Server server) {
        ...
        if (!isPluginPreferred || !isPluginEnabled) {
            if (isPluginPreferred && !this.enabled) {
                this.logger.log(Level.INFO, "已优先使用 spark 插件，但该插件未加载。将改为启用内置的 spark 性能分析器。");
            }
            this.enable();
        }
    }
        ...
        if (!this.enabled) {
            if (GlobalConfiguration.get().spark.enabled) {
                ...
                if (!this.disabledInConfigurationWarningLogged) {
                    this.logger.log(Level.INFO, "spark 性能分析器未启用，因为当前配置中已将其禁用。");
    ...
    public static final class CommandImpl extends Command {
        ...
        public boolean execute(final CommandSender sender, final String commandLabel, final String[] args) {
            ...
            if (spark.enabled) {
                ...
                sender.sendMessage(Component.text("spark 性能分析器当前已禁用。", TextColor.color(SPARK_YELLOW)));
            }
            return true;
        }
