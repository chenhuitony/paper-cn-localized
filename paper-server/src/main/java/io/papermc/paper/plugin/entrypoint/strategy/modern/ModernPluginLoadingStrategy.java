    ...
    public List<ProviderPair<T>> loadProviders(List<PluginProvider<T>> pluginProviders, MetaDependencyTree dependencyTree) {
        ...
        for (PluginProvider<T> provider : pluginProviders) {
            ...
            if (missingDependencies.isEmpty()) {
                ...
                LOGGER.error("无法在 '%s' 中加载 '%s'".formatted(provider.getSource(), provider.getParentSource()), new UnknownDependencyException(missingDependencies, configuration.getName())); // Paper
        ...
        for (String providerIdentifier : reversedTopographicSort) {
            ...
            try {
                ...
                LOGGER.error("无法加载文件夹 '%s' 中的插件 '%s'".formatted(retrievedProvider.getFileName(), retrievedProvider.getParentSource()), ex); // Paper
            }
        }
