...
public final class SyncLoadInfoCommand implements PaperSubcommand {
    ...
    private void doSyncLoadInfo(final CommandSender sender, final String[] args) {
        if (!SyncLoadFinder.ENABLED) {
            ...
            sender.sendMessage(text().color(RED).append(text("此命令需要服务器启动参数 '")).append(
                text(systemFlag, WHITE).clickEvent(ClickEvent.copyToClipboard(systemFlag))
                                       .hoverEvent(HoverEvent.showText(text("点击复制系统参数")))).append(
                text("' 才能使用。")));
            return;
        }
        ...
        if (args.length > 0 && args[0].toLowerCase(Locale.ROOT).equals("clear")) {
            ...
            sender.sendMessage(text("同步加载数据已清除。", GRAY));
            return;
        }
        ...
        sender.sendMessage(text("正在将同步加载信息写入 " + file, GREEN));
        ...
        try {
            ...
            sender.sendMessage(text("同步加载信息已成功写入！", GREEN));
        } catch (Throwable thr) {
            sender.sendMessage(text("写入同步加载信息失败！详情请查看控制台。", RED));
