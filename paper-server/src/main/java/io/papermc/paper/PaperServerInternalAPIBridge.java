...
public class PaperServerInternalAPIBridge implements InternalAPIBridge {
    ...
    public DamageEffect getDamageEffect(final String key) {
        return Objects.requireNonNull(CraftDamageEffect.getById(key), "未找到对应的 DamageEffect：" + key + "。这是一个 Bug。");
    }
    ...
    public ItemStack deserializeItem(final byte[] data) {
        ...
        Preconditions.checkArgument(dataVersion <= CraftMagicNumbers.INSTANCE.getDataVersion(), "版本更高！不支持服务端降级！");
