        ...
        if (ConfigurationPart.class.isAssignableFrom(type) && !this.instanceMap.containsKey(type)) {
            try {
                ...
                if (type.getEnclosingClass() != null && !Modifier.isStatic(type.getModifiers())) {
                    ...
                    if (instance == null) {
                        throw new SerializationException("无法创建内部类的新实例 " + type.getName() + "，但没有以下外部类的实例：" + type.getEnclosingClass().getName());
                    }
                ...
                throw new SerializationException(ConfigurationPart.class, target + " 必须是有效的 ConfigurationPart", e);
            }
        } else {
            throw new SerializationException(target + " 必须是有效的 ConfigurationPart");
        }
    }
