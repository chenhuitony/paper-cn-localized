...
public class PaperPotionBrewer implements PotionBrewer {
    ...
    public Collection<PotionEffect> getEffects(PotionType type, boolean upgraded, boolean extended) {
        ...
        Preconditions.checkArgument(!key.getKey().startsWith("strong_"), "强效药水类型不能直接使用，实际为 %s", key);
        Preconditions.checkArgument(!key.getKey().startsWith("long_"), "延长型药水类型不能直接使用，实际为 %s", key);
        ...
        Preconditions.checkNotNull(type, "从数据中得到的未知药水类型 " + effectiveKey.asMinimalString()); // Legacy error message in 1.20.4
