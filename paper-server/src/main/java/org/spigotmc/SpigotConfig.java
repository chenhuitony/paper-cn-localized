...
public class SpigotConfig {
    ...
    private static final String HEADER = """
        这是 Paper 的 Spigot 配置文件。
        如你所见，有非常多的选项可以配置。部分选项可能影响游戏玩法，因此请谨慎使用，
        并在配置前确保你了解每个选项的作用。
        
        如果你在配置方面需要帮助，或有任何与 Paper 相关的问题，
        请加入我们的 Discord 或查阅文档页面。
        
        文件参考: https://docs.papermc.io/paper/reference/spigot-configuration/
    ...
    public static void init(File configFile) {
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "无法加载 spigot.yml，请修正语法错误", ex);
    ...
    public static void readConfig(Class<?> clazz, Object instance) {
            ...
            if (Modifier.isPrivate(method.getModifiers())) {
                if (method.getParameterTypes().length == 0 && method.getReturnType() == Void.TYPE) {
                    try {
                        ...
                        Bukkit.getLogger().log(Level.SEVERE, "调用以下内容时出错：" + method, ex);
                    }
                }
            }
        }
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "无法保存 " + SpigotConfig.CONFIG_FILE, ex);
        }
    }
    ...
    public static String outdatedClientMessage = "客户端版本过旧！请使用 {0}";
    public static String outdatedServerMessage = "服务端版本过旧！我仍在使用 {0}";
    ...
    private static void watchdog() {
        ...
        SpigotConfig.restartMessage = SpigotConfig.transform(SpigotConfig.getString("messages.restart", "服务端正在重启"));
    ...
    private static void bungee() {
        if (SpigotConfig.version < 4) {
            ...
            System.out.println("配置已过时，正在禁用 BungeeCord 支持！");
        }
    ...
    private static void nettyThreads() {
        ...
        Bukkit.getLogger().log(Level.INFO, "基于 Netty 的 IO 使用 {0} 个线程", count);
    }
            ...
            if (section.isInt(name)) {
                try {
                    ...
                    if (BuiltInRegistries.CUSTOM_STAT.get(key) == null) {
                        Bukkit.getLogger().log(Level.WARNING, "正在忽略不存在的 stats.forced-stats " + name);
                        continue;
                    }
                    SpigotConfig.forcedStats.put(key, section.getInt(name));
                } catch (Exception ex) {
                    Bukkit.getLogger().log(Level.WARNING, "正在忽略无效的 stats.forced-stats " + name);
                }
            }
        }
    }
    ...
    private static void playerSample() {
        ...
        Bukkit.getLogger().log(Level.INFO, "服务端 Ping 玩家样本数：{0}", playerSample); // Paper - Use logger
    }
