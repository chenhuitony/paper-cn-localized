...
public final class CraftPersistentDataTypeRegistry {
    ...
    private record TagAdapter<P, T extends Tag>(
        ...
        private P extract(final PersistentDataType<P, ?> dataType, final Tag base) {
            Preconditions.checkArgument(this.tagType.isInstance(base), "提供的 Tag 类型为 %s，应为 %s", base.getClass().getSimpleName(), this.tagType.getSimpleName());
        ...
        private T build(final PersistentDataType<P, ?> dataType, final Object value) {
            Preconditions.checkArgument(this.primitiveType.isInstance(value), "提供的值类型为 %s，应为 %s", value.getClass().getSimpleName(), this.primitiveType.getSimpleName());
    ...
    private <T> TagAdapter createAdapter(Class<T> type) {
        ...
        throw new IllegalArgumentException("找不到适用于所请求类型的有效 TagAdapter 实现：" + type.getSimpleName());
    }
    ...
    public <T, Z extends Tag> T extract(final PersistentDataType<T, ?> type, final Tag tag) throws ClassCastException, IllegalArgumentException {
        ...
        Preconditions.checkArgument(adapter.isInstance(type, tag), "找到的标签实例（%s）不能存储 %s", tag.getClass().getSimpleName(), primitiveType.getSimpleName());

        final Object foundValue = adapter.extract(type, tag);
        Preconditions.checkArgument(primitiveType.isInstance(foundValue), "找到的对象类型为 %s，应为 %s", foundValue.getClass().getSimpleName(), primitiveType.getSimpleName());
    ...
    private <P, T extends List<P>> ListTag constructList(@NotNull final PersistentDataType<T, ?> type, @NotNull final List<P> list) {
        Preconditions.checkArgument(type instanceof ListPersistentDataType<?, ?>, "传入的列表不能用 %s 写入 PDC（应为列表数据类型）", type.getClass().getSimpleName());
    ...
    private <P> List<P> extractList(
        ...
        Preconditions.checkArgument(type instanceof ListPersistentDataType<?, ?>, "找到的列表标签不能用 %s 读取（应为列表数据类型）", type.getClass().getSimpleName());
        final ListPersistentDataType<P, ?> listPersistentDataType = (ListPersistentDataType<P, ?>) type;
