...
public class CraftLimitedRegion extends CraftRegionAccessor implements LimitedRegion {
    ...
    public WorldGenLevel getHandle() {
        ...
        Preconditions.checkState(handle != null, "WorldGenLevel 已不存在，你是否在另一个 tick 中使用它？");
    ...
    public void saveEntities() {
        ...
        for (net.minecraft.world.entity.Entity entity : this.entities) {
            if (entity.isAlive()) {
                ...
                Preconditions.checkState(this.region.contains(entity.getX(), entity.getY(), entity.getZ()), "实体 %s 不在区域内", entity);
    ...
    public Biome getBiome(int x, int y, int z) {
        Preconditions.checkArgument(this.isInRegion(x, y, z), "坐标 %s, %s, %s 不在区域内", x, y, z);
    ...
    public int getHighestBlockYAt(int x, int z) {
        Preconditions.checkArgument(this.isInRegion(x, this.region.getCenter().getBlockY(), z), "坐标 %s, %s 不在区域内", x, z);
        return super.getHighestBlockYAt(x, z);
    }

    @Override
    public int getHighestBlockYAt(Location location) {
        Preconditions.checkArgument(this.isInRegion(location), "坐标 %s, %s, %s 不在区域内", location.getBlockX(), location.getBlockY(), location.getBlockZ());
    ...
    public void setBlockState(int x, int y, int z, BlockState state) { // todo deprecate, this doesn't make sense plugin should use the update method after fetching it from getBlockState
        ...
        if (!(state instanceof CraftBlockEntityState<?> entityState)) {
            throw new IllegalArgumentException("state 不是有效的方块实体（TileState）");
        }

        BlockPos pos = new BlockPos(x, y, z);
        if (!state.getBlockData().matches(this.getHandle().getBlockState(pos).asBlockData())) {
            throw new IllegalArgumentException("BlockData 不匹配！期望 " + state.getBlockData().getAsString(false) + "，得到 " + getHandle().getBlockState(pos).asBlockData().getAsString(false));
        }
