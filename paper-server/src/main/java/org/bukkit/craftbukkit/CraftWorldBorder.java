...
public class CraftWorldBorder implements WorldBorder {
    ...
    public void setSize(double newSize) {
        Preconditions.checkArgument(newSize >= 1.0 && newSize <= this.getMaxSize(), "newSize 必须在 1.0 到 %s 之间", this.getMaxSize());
        this.handle.setSize(newSize);
    }

    @Override
    public void changeSize(double newSize, long ticks) {
        Preconditions.checkArgument(ticks >= 0 && ticks <= Integer.MAX_VALUE, "ticks 必须在 0-%s 之间", Integer.MAX_VALUE);
        Preconditions.checkArgument(newSize >= 1.0 && newSize <= this.getMaxSize(), "newSize 必须在 1.0 到 %s 之间", this.getMaxSize());
    ...
    public void setCenter(double x, double z) {
        Preconditions.checkArgument(Math.abs(x) <= this.getMaxCenterCoordinate(), "x 坐标不能超出 ±%s", this.getMaxCenterCoordinate());
        Preconditions.checkArgument(Math.abs(z) <= this.getMaxCenterCoordinate(), "z 坐标不能超出 ±%s", this.getMaxCenterCoordinate());
    ...
    public void setWarningTimeTicks(final int ticks) {
        Preconditions.checkArgument(ticks >= 0, "ticks 不能小于 0");
