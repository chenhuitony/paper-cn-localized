...
public final class CraftPlayerTextures implements PlayerTextures {
    ...
    private static void validateTextureUrl(@Nullable URL url) {
        ...
        Preconditions.checkArgument(url.getHost().equals(CraftPlayerTextures.MINECRAFT_HOST), "期望主机为 '%s'，但实际为 '%s'", CraftPlayerTextures.MINECRAFT_HOST, url.getHost());
        Preconditions.checkArgument(url.getPath().startsWith(CraftPlayerTextures.MINECRAFT_PATH), "期望路径以 '%s' 开头，但实际为 '%s", CraftPlayerTextures.MINECRAFT_PATH, url.getPath());
    }
    ...
    public void copyFrom(PlayerTextures other) {
        ...
        Preconditions.checkArgument(other instanceof CraftPlayerTextures, "期望得到 CraftPlayerTextures，但实际为 %s", other.getClass().getName());
