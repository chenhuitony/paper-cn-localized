...
public final class CraftScoreboard implements org.bukkit.scoreboard.Scoreboard {
    ...
    public Objective registerNewObjective(String name, Criteria criteria, net.kyori.adventure.text.Component displayName, RenderType renderType) throws IllegalArgumentException {
        ...
        Preconditions.checkArgument(name != null, "记分板目标名称不能为 null");
        Preconditions.checkArgument(criteria != null, "条件不能为 null");
        Preconditions.checkArgument(renderType != null, "RenderType 不能为 null");
        Preconditions.checkArgument(name.length() <= Short.MAX_VALUE, "名称 '%s' 超过了 32767 个字符的限制（%s）", name, name.length());
        Preconditions.checkArgument(this.getHandle().getObjective(name) == null, "名为 '%s' 的记分板目标已存在", name);
    ...
    public ImmutableSet<Objective> getObjectivesByCriteria(String criteria) {
        Preconditions.checkArgument(criteria != null, "条件名不能为 null");
    ...
    public Objective getObjective(DisplaySlot slot) {
        Preconditions.checkArgument(slot != null, "显示槽位不能为 null");
    ...
    private ImmutableSet<Score> getScores(ScoreHolder entry) {
        Preconditions.checkArgument(entry != null, "条目不能为 null");
    ...
    public Team getPlayerTeam(OfflinePlayer player) {
        Preconditions.checkArgument(player != null, "OfflinePlayer 不能为 null");
    ...
    public Team getTeam(String teamName) {
        Preconditions.checkArgument(teamName != null, "队伍名称不能为 null");
    ...
    public Team registerNewTeam(String name) {
        Preconditions.checkArgument(name != null, "队伍名称不能为 null");
        Preconditions.checkArgument(name.length() <= Short.MAX_VALUE, "队伍名称 '%s' 超过了 32767 个字符的限制（%s）", name, name.length());
        Preconditions.checkArgument(this.getHandle().getPlayerTeam(name) == null, "队伍名称 '%s' 已被使用", name);
    ...
    public void clearSlot(DisplaySlot slot) {
        Preconditions.checkArgument(slot != null, "槽位不能为 null");
