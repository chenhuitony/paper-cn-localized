...
public class CraftDecoratedPot extends CraftBlockEntityState<DecoratedPotBlockEntity> implements DecoratedPot {
    ...
    public void setSherd(Side face, Material sherd) {
        ...
        Preconditions.checkArgument(sherd == null || sherd == Material.BRICK || Tag.ITEMS_DECORATED_POT_SHERDS.isTagged(sherd), "sherd 不是有效的饰陶碎片材料：%s", sherd);
        ...
        switch (face) {
            ...
            default -> throw new IllegalArgumentException("意外的值：" + face);
        }
    }
    ...
    public void startWobble(@NotNull final WobbleStyle style) {
        Preconditions.checkArgument(style != null, "style 不能为 null");
        this.requirePlaced();
