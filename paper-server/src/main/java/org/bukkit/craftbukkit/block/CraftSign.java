...
public class CraftSign<T extends SignBlockEntity> extends CraftBlockEntityState<T> implements Sign {
    ...
    public SignSide getSide(Side side) {
        Preconditions.checkArgument(side != null, "side 为 null");
    ...
    public static void openSign(Sign sign, Player player, Side side) {
        Preconditions.checkArgument(sign != null, "sign 为 null");
        Preconditions.checkArgument(side != null, "side 为 null");
        Preconditions.checkArgument(sign.isPlaced(), "告示牌必须已放置");
        Preconditions.checkArgument(sign.getWorld() == player.getWorld(), "告示牌必须与玩家位于同一世界");
