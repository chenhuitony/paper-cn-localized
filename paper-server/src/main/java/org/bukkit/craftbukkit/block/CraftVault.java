...
public class CraftVault extends CraftBlockEntityState<VaultBlockEntity> implements Vault {
    ...
    public void setActivationRange(final double activationRange) {
        Preconditions.checkArgument(Double.isFinite(activationRange), "激活范围不能为 NaN 或无穷大");
        Preconditions.checkArgument(activationRange <= this.getDeactivationRange(), "新加激范围（%s）必须小于或等于停用范围（%s）", activationRange, this.getDeactivationRange());
    ...
    public void setDeactivationRange(final double deactivationRange) {
        Preconditions.checkArgument(Double.isFinite(deactivationRange), "停用范围不能为 NaN 或无穷大");
        Preconditions.checkArgument(deactivationRange >= this.getActivationRange(), "新停用范围（%s）必须大于或等于加激范围（%s）", deactivationRange, this.getActivationRange());
    ...
    public void setKeyItem(final ItemStack key) {
        Preconditions.checkArgument(key != null, "key 不能为 null");
    ...
    public void setLootTable(final LootTable lootTable) {
        ...
        Preconditions.checkArgument(lootTableKey != null, "lootTable 不能为 null");
    ...
    public boolean addRewardedPlayer(final UUID playerUUID) {
        Preconditions.checkArgument(playerUUID != null, "playerUUID 不能为 null");
    ...
    public void setDisplayedItem(final ItemStack displayedItem) {
        Preconditions.checkArgument(displayedItem != null, "displayedItem 不能为 null");
