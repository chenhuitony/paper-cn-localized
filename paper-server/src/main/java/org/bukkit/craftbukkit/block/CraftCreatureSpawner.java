...
public class CraftCreatureSpawner extends CraftBlockEntityState<SpawnerBlockEntity> implements CreatureSpawner, org.bukkit.craftbukkit.spawner.PaperSharedSpawnerLogic { // Paper - more spawner API
    ...
    public void setSpawnedType(EntityType entityType) {
        ...
        Preconditions.checkArgument(entityType != EntityType.UNKNOWN, "无法从刷怪笼生成实体类型 %s！", entityType);
    ...
    public void setSpawnedEntity(SpawnerEntry spawnerEntry) {
        Preconditions.checkArgument(spawnerEntry != null, "条目不能为 null");
    ...
    public static void addPotentialSpawn(BaseSpawner spawner, EntitySnapshot snapshot, int weight, SpawnRule spawnRule, SpawnerEntry.Equipment equipment) {
        Preconditions.checkArgument(snapshot != null, "快照不能为 null");
    ...
    public void setMinSpawnDelay(int spawnDelay) {
        Preconditions.checkArgument(spawnDelay <= this.getMaxSpawnDelay(), "最小生成延迟必须小于或等于最大生成延迟");
    ...
    public void setMaxSpawnDelay(int spawnDelay) {
        Preconditions.checkArgument(spawnDelay > 0, "最大生成延迟必须大于 0。");
        Preconditions.checkArgument(spawnDelay >= this.getMinSpawnDelay(), "最大生成延迟必须大于或等于最小生成延迟");
