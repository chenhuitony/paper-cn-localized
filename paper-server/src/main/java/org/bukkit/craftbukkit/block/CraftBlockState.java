...
public class CraftBlockState implements BlockState {
    ...
    protected final void ensureNoWorldGeneration() {
        Preconditions.checkState(!this.isWorldGeneration(), "在世界生成期间不支持此操作！");
    }
    ...
    public void setData(final MaterialData data) {
        ...
        if (mat != null) {
            Preconditions.checkArgument((data.getClass() == mat.getData()) || (data.getClass() == MaterialData.class), "提供的数据不是 %s 类型，实际为 %s", mat.getData().getName(), data.getClass().getName());
        }
    ...
    public void setType(final Material type) {
        ...
        Preconditions.checkArgument(type.isBlock(), "材料必须是方块！");
    ...
    protected void requirePlaced() {
        Preconditions.checkState(this.isPlaced(), "必须先放置该 blockState 才能调用此方法");
    }
