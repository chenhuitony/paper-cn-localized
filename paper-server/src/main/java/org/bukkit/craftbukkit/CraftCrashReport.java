...
public class CraftCrashReport implements CrashReportDetail<Object> {
    ...
    public String call() {
        ...
        try {
            value.append("\n   BrandInfo: ").append(String.format("%s（%s）版本 %s", build.brandName(), build.brandId(), build.asString(io.papermc.paper.ServerBuildInfo.StringRepresentation.VERSION_FULL)));
            value.append("\n   Running: ").append(Bukkit.getName()).append(" version ").append(Bukkit.getVersion()).append("（实现 API 版本 ").append(Bukkit.getBukkitVersion()).append(") ").append(String.valueOf(MinecraftServer.getServer().usesAuthentication()));
            value.append("\n   Plugins: {");
