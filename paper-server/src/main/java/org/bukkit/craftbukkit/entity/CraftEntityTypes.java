...
public final class CraftEntityTypes {
    ...
    static {
        ...
        register(new EntityTypeData<>(EntityType.ENDER_DRAGON, EnderDragon.class, CraftEnderDragon::new, spawnData -> {
            Preconditions.checkArgument(spawnData.normalWorld(), "世界生成期间无法生成实体 %s", EnderDragon.class.getName());
    ...
    private static void register(EntityTypeData<?, ?> typeData) {
        ...
        if (other != null) {
            Bukkit.getLogger().warning(String.format("为类 %s 找到多个实体类型数据，正在用新值 '%s' 替换 '%s'", typeData.entityClass().getName(), other, typeData));
        }

        other = CraftEntityTypes.ENTITY_TYPE_DATA.put(typeData.entityType(), typeData);
        if (other != null) {
            Bukkit.getLogger().warning(String.format("为实体类型 %s 找到多个实体类型数据，正在用新值 '%s' 替换 '%s'", typeData.entityType().getKey(), other, typeData));
        }
    }
