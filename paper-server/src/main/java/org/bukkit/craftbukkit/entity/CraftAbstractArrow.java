...
public abstract class CraftAbstractArrow extends AbstractProjectile implements AbstractArrow {
    ...
    public void setDamage(double damage) {
        Preconditions.checkArgument(damage >= 0, "伤害值 (%s) 必须为正数", damage);
    ...
    public void setPierceLevel(int pierceLevel) {
        Preconditions.checkArgument(0 <= pierceLevel && pierceLevel <= Byte.MAX_VALUE, "穿透等级（%s）超出范围，应为 0 < 等级 < 127", pierceLevel);
    ...
    public void setPickupStatus(PickupStatus status) {
        Preconditions.checkArgument(status != null, "拾取状态不能为 null");
