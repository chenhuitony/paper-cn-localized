...
public class CraftFox extends CraftAnimals implements Fox {
    ...
    public void setFirstTrustedPlayer(AnimalTamer player) {
        if (player == null) {
            Preconditions.checkState(this.getHandle().getEntityData().get(net.minecraft.world.entity.animal.fox.Fox.DATA_TRUSTED_ID_1).isEmpty(), "必须先移除第二个受信任玩家");
        }
    ...
    public void setSecondTrustedPlayer(AnimalTamer player) {
        if (player != null) {
            Preconditions.checkState(this.getHandle().getEntityData().get(net.minecraft.world.entity.animal.fox.Fox.DATA_TRUSTED_ID_0).isPresent(), "必须先添加第一个受信任玩家");
        }
