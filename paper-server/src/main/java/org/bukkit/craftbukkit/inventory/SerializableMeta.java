...
public final class SerializableMeta implements ConfigurationSerializable {
    ...
    public static ItemMeta deserialize(Map<String, Object> map) throws Throwable {
        Preconditions.checkArgument(map != null, "无法反序列化 null 映射");
        ...
        if (constructor == null) {
            throw new IllegalArgumentException(type + " 不是有效的 " + SerializableMeta.TYPE_FIELD);
        }
    ...
    public static <T> T getObject(Class<T> clazz, Map<?, ?> map, Object field, boolean nullable) {
        ...
        if (object == null) {
            if (!nullable) {
                throw new NoSuchElementException(map + " 不包含 " + field);
            }
            return null;
        }
        throw new IllegalArgumentException(field + "(" + object + ") 不是有效的 " + clazz);
    }
