...
public class CraftInventoryView<T extends AbstractContainerMenu, I extends Inventory> extends CraftAbstractInventoryView {
    ...
    public static void sendInventoryTitleChange(InventoryView view, String title) {
        Preconditions.checkArgument(view != null, "InventoryView 不能为空");
        Preconditions.checkArgument(title != null, "标题不能为 null");
        Preconditions.checkArgument(view.getPlayer() instanceof Player, "此功能目前不支持 NPC");
        Preconditions.checkArgument(view.getTopInventory().getType().isCreatable(), "只有可创建的物品栏才能更改其标题");
