...
public class CraftMetaFirework extends CraftMetaItem implements FireworkMeta {
    ...
    void safelyAddEffects(Iterable<?> collection, final boolean throwOnOversize) {
        ...
        for (Object obj : collection) {
            Preconditions.checkArgument(obj instanceof FireworkEffect, "%s 中的 %s 不是 FireworkEffect", obj, collection);
            if (effects.size() + 1 > Fireworks.MAX_EXPLOSIONS) {
                if (throwOnOversize) {
                    throw new IllegalArgumentException("数量不能超过 " + Fireworks.MAX_EXPLOSIONS + " 个烟花效果");
                } else {
                    continue;
                }
            }
    ...
    public void addEffect(FireworkEffect effect) {
        Preconditions.checkArgument(effect != null, "FireworkEffect 不能为 null");
        Preconditions.checkArgument(this.effects == null || this.effects.size() + 1 <= Fireworks.MAX_EXPLOSIONS, "烟花效果不能超过 %s 个", Fireworks.MAX_EXPLOSIONS); // Paper - limit firework effects
        if (this.effects == null) {
    ...
    public void addEffects(FireworkEffect... effects) {
        Preconditions.checkArgument(effects != null, "effects 不能为 null");
        // Paper start - limit firework effects
        final int initialSize = this.effects == null ? 0 : this.effects.size();
        Preconditions.checkArgument(initialSize + effects.length <= Fireworks.MAX_EXPLOSIONS, "烟花效果数量不能超过 %s", Fireworks.MAX_EXPLOSIONS);
        ...
        for (FireworkEffect effect : effects) {
            Preconditions.checkArgument(effect != null, "effects 不能包含 null 的 FireworkEffect");
            list.add(effect);
        }
    }
    ...
    public void removeEffect(int index) {
        if (this.effects == null) {
            throw new IndexOutOfBoundsException("Index: " + index + ", 大小: 0");
        } else {
    ...
    public void setPower(int power) {
        Preconditions.checkArgument(power >= 0, "power 不能小于 0：%s", power);
        Preconditions.checkArgument(power <= 255, "power 不能大于 255：%s", power);
        this.power = power;
    }
}
