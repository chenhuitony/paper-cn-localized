...
public class CraftInventoryPlayer extends CraftInventory implements org.bukkit.inventory.PlayerInventory, EntityEquipment {
    ...
    public void setItem(int index, ItemStack item) {
        ...
        if (index < 0 || index > 42) {
            throw new ArrayIndexOutOfBoundsException("索引必须介于 0 到 42 之间");
        }
    ...
    public void setItem(EquipmentSlot slot, ItemStack item) {
        Preconditions.checkArgument(slot != null, "slot 不能为 null");
    ...
    public void setHeldItemSlot(int slot) {
        Preconditions.checkArgument(slot >= 0 && slot < Inventory.getSelectionSize(), "槽位（%s）不在 0 到 %s（含）之间", slot, Inventory.getSelectionSize() - 1);
    ...
    private void setSlots(ItemStack[] items, int baseSlot, int length) {
        ...
        Preconditions.checkArgument(items.length <= length, "items.length 必须 <= %s", length);
    ...
    public void setItemInMainHandDropChance(float chance) {
        throw new UnsupportedOperationException("无法为玩家物品栏设置掉落几率");
    }
