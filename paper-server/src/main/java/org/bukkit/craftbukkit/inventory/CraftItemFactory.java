...
public final class CraftItemFactory implements ItemFactory {
    ...
    public boolean isApplicable(ItemMeta meta, Material type) {
        ...
        Preconditions.checkArgument(meta instanceof CraftMetaItem, "%s 的 Meta 并非由 %s 创建", meta.getClass().toString(), CraftItemFactory.class.getName());
    ...
    public boolean equals(ItemMeta meta1, ItemMeta meta2) {
        ...
        if (meta1 != null) {
            Preconditions.checkArgument(meta1 instanceof CraftMetaItem, "%s 的第一个 meta 不属于 %s", meta1.getClass().getName(), CraftItemFactory.class.getName());
        } else {
            return ((CraftMetaItem) meta2).isEmpty();
        }
        if (meta2 != null) {
            Preconditions.checkArgument(meta2 instanceof CraftMetaItem, "%s 的第二个 Meta 不属于 %s", meta2.getClass().getName(), CraftItemFactory.class.getName());
        } else {
    ...
    public ItemMeta asMetaFor(ItemMeta meta, ItemStack stack) {
        Preconditions.checkArgument(stack != null, "ItemStack 不能为 null");
    ...
    public ItemMeta asMetaFor(ItemMeta meta, Material material) {
        Preconditions.checkArgument(material != null, "材料不能为 null");
        Preconditions.checkArgument(meta instanceof CraftMetaItem, "%s 的 ItemMeta 并非由 %s 创建", (meta != null ? meta.getClass().toString() : "null"), CraftItemFactory.class.getName());
    ...
    public ItemStack createItemStack(String input) throws IllegalArgumentException {
        try {
            ...
            if (reader.canRead()) {
                throw new IllegalArgumentException("解析 ItemStack 时发现尾部多余输入：" + reader.getRemaining());
            }
            return CraftItemStack.asCraftMirror(in.createItemStack(1));
        } catch (CommandSyntaxException ex) {
            throw new IllegalArgumentException("无法解析物品堆：" + input, ex);
        }
    }
    ...
    public ItemStack enchantItem(final World world, final ItemStack itemStack, final int level, final boolean allowTreasures) {
        Preconditions.checkArgument(world != null, "世界不能为 null");
    ...
    private static ItemStack enchantItem(RandomSource source, ItemStack itemStack, int level, boolean allowTreasures) {
        ...
        Preconditions.checkArgument(!itemStack.getType().isAir(), "ItemStack 不能为空气");
    ...
    public net.kyori.adventure.text.event.HoverEvent<net.kyori.adventure.text.event.HoverEvent.ShowItem> asHoverEvent(final ItemStack item, final java.util.function.UnaryOperator<net.kyori.adventure.text.event.HoverEvent.ShowItem> op) {
        Preconditions.checkArgument(item.getAmount() > 0 && item.getAmount() <= Item.ABSOLUTE_MAX_STACK_SIZE, "ItemStack 数量必须在 1 到 %s 之间，但实际为 %s", Item.ABSOLUTE_MAX_STACK_SIZE, item.getAmount());
    ...
    public net.md_5.bungee.api.chat.hover.content.Content hoverContentOf(ItemStack itemStack) {
        throw new UnsupportedOperationException("BungeeCord 聊天 API 不支持数据组件");
        /*
    ...
    private ItemStack enchantWithLevels(
        ...
        Preconditions.checkArgument(itemStack != null, "参数 'itemStack' 不能为 null");
        Preconditions.checkArgument(!itemStack.isEmpty(), "参数 'itemStack' 不能为空");
        Preconditions.checkArgument(random != null, "参数 'random' 不能为 null");
