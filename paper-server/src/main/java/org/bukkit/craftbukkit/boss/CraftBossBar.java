...
public class CraftBossBar implements BossBar {
    ...
    public void setProgress(double progress) {
        Preconditions.checkArgument(progress >= 0.0 && progress <= 1.0, "进度必须在 0.0 到 1.0 之间（%s）", progress);
    ...
    public void addPlayer(Player player) {
        Preconditions.checkArgument(player != null, "player 为 null");
        Preconditions.checkArgument(((CraftPlayer) player).getHandle().connection != null, "玩家尚未完全连接（请等待 PlayerJoinEvent）");
