...
public abstract class CraftRegionAccessor implements RegionAccessor {
    ...
    public void setBiome(int x, int y, int z, Biome biome) {
        ...
        Preconditions.checkArgument(b != null, "无法将生物群系设置为 %s", biome);
    ...
    public <T extends Entity> T addEntity(T entity) {
        Preconditions.checkArgument(!entity.isInWorld(), "实体已被添加到某个世界");
        net.minecraft.world.entity.Entity nmsEntity = ((CraftEntity) entity).getHandle();
        if (nmsEntity.level() != this.getHandle().getLevel()) {
            throw new IllegalArgumentException(entity + " 不是用这个世界创建的，你必须用要添加到的那个世界来创建实体。");
        }
    ...
    public <T extends Entity> T addEntity(net.minecraft.world.entity.Entity entity, CreatureSpawnEvent.SpawnReason reason, Consumer<? super T> function, boolean randomizeData) throws IllegalArgumentException {
        Preconditions.checkArgument(entity != null, "无法生成 null 实体");
    ...
    public net.minecraft.world.entity.Entity createEntity(Location location, Class<? extends Entity> clazz, boolean randomizeData) throws IllegalArgumentException {
        ...
        Preconditions.checkArgument(clazz != null, "实体类不能为 null");
        ...
        if (entityTypeData == null || entityTypeData.spawnFunction() == null) {
            if (CraftEntity.class.isAssignableFrom(clazz)) {
                ...
                throw new IllegalArgumentException(String.format("无法从 CraftBukkit 实现类 '%s' 生成实体，请改用 Bukkit 类。"
                        + "你可以通过 Entity#getType()#getEntityClass() 获取 Bukkit 表示", clazz.getName()));
            } else {
                throw new IllegalArgumentException("无法为以下类型生成实体：" + clazz.getName());
            }
        }

        if (!this.isEnabled(entityTypeData.entityType())) {
            throw new IllegalArgumentException("无法为以下类型生成实体：" + clazz.getName() + "，因为它不是已启用的特性");
        }
        ...
        if (this.getHandle().getDifficulty() == Difficulty.PEACEFUL && !CraftEntityType.bukkitToMinecraft(entityTypeData.entityType()).isAllowedInPeaceful()) {
            throw new IllegalStateException("无法为以下类型生成怪物：" + clazz.getName() + " 在和平难度下");
        }
    ...
    public boolean lineOfSightExists(Location from, Location to) {
        Preconditions.checkArgument(from != null, "lineOfSightExists 中的 from 参数不能为 null");
        Preconditions.checkArgument(to != null, "lineOfSightExists 的 to 参数不能为 null");
