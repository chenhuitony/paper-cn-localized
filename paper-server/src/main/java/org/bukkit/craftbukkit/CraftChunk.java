...
public class CraftChunk implements Chunk {
    ...
    public Collection<BlockState> getTileEntities(Predicate<? super Block> blockPredicate, boolean useSnapshot) {
        Preconditions.checkArgument(blockPredicate != null, "blockPredicate 不能为 null");
    ...
    public void setInhabitedTime(long ticks) {
        Preconditions.checkArgument(ticks >= 0, "ticks 不能为负数");
    ...
    static void validateChunkCoordinates(int minY, int maxY, int x, int y, int z) {
        Preconditions.checkArgument(0 <= x && x <= 15, "x 超出范围（应为 0-15，实际为 %s）", x);
        Preconditions.checkArgument(minY <= y && y <= maxY, "y 超出范围（应为 %s-%s，实际为 %s）", minY, maxY, y);
        Preconditions.checkArgument(0 <= z && z <= 15, "z 超出范围（应为 0-15，实际为 %s）", z);
    }
