...
public class CraftScheduler implements BukkitScheduler {
    ...
    */ // Paper end
    ...
    public void cancelTasks(final Plugin plugin) {
        Preconditions.checkArgument(plugin != null, "无法取消 null 插件的任务");
        // Paper start
        if (!this.isAsyncScheduler) {
    ...
    public void mainThreadHeartbeat() {
        ...
        while (this.isReady(this.currentTick)) {
            ...
            if (task.isSync()) {
                ...
                task.getOwner().getLogger().log(Level.SEVERE, "同步调度器中出现意外的异步任务。请向 Paper 报告此问题"); // Paper
    ...
    private static void validate(final Plugin plugin, final Object task) {
        ...
        Preconditions.checkArgument(task instanceof Runnable || task instanceof Consumer || task instanceof Callable, "任务必须是 Runnable、Consumer 或 Callable");
        if (!plugin.isEnabled()) {
            throw new IllegalPluginAccessException("插件在禁用状态下尝试注册任务");
        }
    }

    private int nextId() {
        Preconditions.checkArgument(this.runners.size() < Integer.MAX_VALUE, "已调度了 %s 个任务！无法再调度更多", Integer.MAX_VALUE);
        int id;
        do {
    ...
    public int scheduleSyncDelayedTask(Plugin plugin, BukkitRunnable task, long delay) {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTaskLater(Plugin, long)");
    }

    @Deprecated
    @Override
    public int scheduleSyncDelayedTask(Plugin plugin, BukkitRunnable task) {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTask(Plugin)");
    }

    @Deprecated
    @Override
    public int scheduleSyncRepeatingTask(Plugin plugin, BukkitRunnable task, long delay, long period) {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTaskTimer(Plugin, long, long)");
    }
    ...
    public BukkitTask runTaskAsynchronously(Plugin plugin, BukkitRunnable task) throws IllegalArgumentException {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTaskAsynchronously(Plugin)");
    }
    ...
    public BukkitTask runTaskLaterAsynchronously(Plugin plugin, BukkitRunnable task, long delay) throws IllegalArgumentException {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTaskLaterAsynchronously(Plugin, long)");
    }
    ...
    public BukkitTask runTaskTimerAsynchronously(Plugin plugin, BukkitRunnable task, long delay, long period) throws IllegalArgumentException {
        throw new UnsupportedOperationException("请使用 BukkitRunnable#runTaskTimerAsynchronously(Plugin, long, long)");
    }
    ...
    public Executor getMainThreadExecutor(Plugin plugin) {
        Preconditions.checkArgument(plugin != null, "插件不能为 null");
        return command -> {
            Preconditions.checkArgument(command != null, "命令不能为 null");
