...
public final class CraftRaid implements Raid {
    ...
    public void setBadOmenLevel(int badOmenLevel) {
        ...
        Preconditions.checkArgument(0 <= badOmenLevel && badOmenLevel <= max, "不祥之兆等级必须在 0 到 %s 之间", max);
    ...
    public void setTotalWaves(int waves) {
        Preconditions.checkArgument(waves > 0, "总波数必须大于 0");
        Preconditions.checkArgument(waves <= 7, "总波数不能大于 7");
        Preconditions.checkArgument(waves >= this.getSpawnedGroups(), "总波数必须大于或等于当前已生成的组数（%s）", this.getSpawnedGroups());
