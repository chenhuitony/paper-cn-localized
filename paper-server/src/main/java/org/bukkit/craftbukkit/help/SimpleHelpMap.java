...
public class SimpleHelpMap implements HelpMap {
    ...
    public SimpleHelpMap(CraftServer server) {
        ...
        this.defaultTopic = new IndexHelpTopic("Index", null, null, Collections2.filter(this.helpTopics.values(), indexFilter), "使用 /help [n] 查看帮助的第 n 页。");
    ...
    public synchronized void initializeCommands() {
        ...
        if (!filteredTopics.isEmpty()) {
            this.addTopic(new IndexHelpTopic("Aliases", "列出命令别名", null, filteredTopics));
        }
        ...
        for (Map.Entry<String, Set<HelpTopic>> entry : pluginIndexes.entrySet()) {
            this.addTopic(new IndexHelpTopic(entry.getKey(), "所有命令：" + entry.getKey(), null, entry.getValue(), "以下是所有 " + entry.getKey() + " commands:"));
        }
    ...
    public void registerHelpTopicFactory(Class commandClass, HelpTopicFactory factory) {
        Preconditions.checkArgument(Command.class.isAssignableFrom(commandClass) || CommandExecutor.class.isAssignableFrom(commandClass), "commandClass（%s）必须实现 Command 或 CommandExecutor", commandClass.getName());
