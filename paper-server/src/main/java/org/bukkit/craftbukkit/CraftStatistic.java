...
public enum CraftStatistic {
    ...
    private CraftStatistic(Identifier key) {
        ...
        Preconditions.checkState(this.bukkit != null, "Bukkit 统计 %s 不存在", this.name());
    }

    public static org.bukkit.Statistic getBukkitStatistic(net.minecraft.stats.Stat<?> statistic) {
        Preconditions.checkArgument(statistic != null, "NMS 统计项不能为 null");
    ...
    public static net.minecraft.stats.Stat getNMSStatistic(org.bukkit.Statistic bukkit) {
        Preconditions.checkArgument(bukkit.getType() == Statistic.Type.UNTYPED, "此方法只接受无类型的统计信息");

        net.minecraft.stats.Stat<Identifier> nms = Stats.CUSTOM.get(statistics.inverse().get(bukkit));
        Preconditions.checkArgument(nms != null, "NMS 统计项 %s 不存在", bukkit);

        return nms;
    }

    private static net.minecraft.stats.Stat getBlockTypeStatistic(org.bukkit.Statistic stat, BlockType blockType) {
        Preconditions.checkArgument(blockType != null, "BlockType 不能为 null");
        try {
    ...
    private static net.minecraft.stats.Stat getItemTypeStatistic(org.bukkit.Statistic stat, ItemType itemType) {
        Preconditions.checkArgument(itemType != null, "ItemType 不能为 null");
        try {
    ...
    public static net.minecraft.stats.Stat getEntityStatistic(org.bukkit.Statistic stat, EntityType entity) {
        Preconditions.checkArgument(entity != null, "EntityType 不能为 null");
    ...
    public static int getStatistic(ServerStatsCounter manager, Statistic statistic) {
        Preconditions.checkArgument(statistic != null, "统计项不能为 null");
        Preconditions.checkArgument(statistic.getType() == Type.UNTYPED, "该统计项必须提供额外参数");
    ...
    public static void setStatistic(ServerStatsCounter manager, Statistic statistic, int newValue, ServerPlayer player) {
        Preconditions.checkArgument(statistic != null, "统计项不能为 null");
        Preconditions.checkArgument(statistic.getType() == Type.UNTYPED, "该统计项必须提供额外参数");
        Preconditions.checkArgument(newValue >= 0, "值必须大于或等于 0");
    ...
    public static int getStatistic(ServerStatsCounter manager, Statistic statistic, Material material) {
        ...
        if (statistic.getType() == Type.BLOCK) {
            Preconditions.checkArgument(material.isBlock(), "材料 %s 必须是方块", material);
            ...
            Preconditions.checkArgument(material.isItem(), "材料 %s 必须是物品", material);
            ItemType itemType = material.asItemType();
            nmsStatistic = CraftStatistic.getItemTypeStatistic(statistic, itemType);
        } else {
            throw new IllegalArgumentException("此统计信息不接受 Material 参数");
        }
        Preconditions.checkArgument(nmsStatistic != null, "提供的材料 %s 没有对应的统计项", material);
    ...
    public static int getStatistic(ServerStatsCounter manager, Statistic statistic, EntityType entityType) {
        Preconditions.checkArgument(statistic != null, "统计项不能为 null");
        Preconditions.checkArgument(entityType != null, "EntityType 不能为 null");
        Preconditions.checkArgument(statistic.getType() == Type.ENTITY, "此统计信息不接受 EntityType 参数");
        net.minecraft.stats.Stat nmsStatistic = CraftStatistic.getEntityStatistic(statistic, entityType);
        Preconditions.checkArgument(nmsStatistic != null, "提供的实体类型 %s 没有对应的统计项", entityType);
        return manager.getValue(nmsStatistic);
    }
