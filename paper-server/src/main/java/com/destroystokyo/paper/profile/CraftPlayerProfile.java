...
public class CraftPlayerProfile implements PlayerProfile, SharedPlayerProfile {
    ...
    public CraftPlayerProfile(GameProfile profile) {
        Preconditions.checkArgument(profile != null, "GameProfile 不能为 null！");
    ...
    public void setProperty(ProfileProperty property) {
        ...
        Preconditions.checkArgument(properties.size() < ExtraCodecs.MAX_PROPERTIES, "无法向玩家档案添加超过 %s 个属性", ExtraCodecs.MAX_PROPERTIES);
    ...
    private static void copyProfileProperties(GameProfile source, GameProfile target, boolean clearTarget) {
        if (source == target) {
            throw new IllegalArgumentException("源玩家资料与目标玩家资料相同（" + source + ")");
        }
    ...
    private static GameProfile createAuthLibProfile(UUID uniqueId, String name) {
        Preconditions.checkArgument(name == null || name.length() <= 16, "名称不能超过 16 个字符");
        Preconditions.checkArgument(name == null || StringUtil.isValidPlayerName(name), "玩家资料名称包含无效字符：%s", name);
        ...
        if (map.containsKey("properties")) {
            for (Object propertyData : (List<?>) map.get("properties")) {
                if (!(propertyData instanceof Map)) {
                    throw new IllegalArgumentException("属性数据（" + propertyData + ") 不是有效的 Map");
                }
