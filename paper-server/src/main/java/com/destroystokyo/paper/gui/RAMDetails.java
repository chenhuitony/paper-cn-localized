...
public class RAMDetails extends JList<String> {
    ...
    public void update() {
        ...
        vector.add("内存使用：" + (data.getUsedMem() / 1024L / 1024L) + " MB (" + (data.getFree() * 100L / data.getMax()) + "% 可用)");
        vector.add("Heap: " + (data.getTotal() / 1024L / 1024L) + " / " + (data.getMax() / 1024L / 1024L) + " mb");
        vector.add("平均 tick: " + DECIMAL_FORMAT.format((double) this.server.getAverageTickTimeNanos() / (double) TimeUtil.NANOSECONDS_PER_MILLISECOND) + " ms");
        vector.add("最近 1 分钟、5 分钟、15 分钟的 TPS：" + String.join(", ", tpsAvg));
        setListData(vector);
    }
