...
public class PluginLoadOrderTest {
    ...
    public void testDependencies() {
        for (PluginProvider<PaperTestPlugin> provider : REGISTERED_PROVIDERS) {
            ...
            Assertions.assertTrue(LOAD_ORDER.containsKey(identifier), "提供者未加载！（%s）".formatted(identifier));
            ...
            for (String hardDependency : pluginMeta.getPluginDependencies()) {
                Assertions.assertTrue(LOAD_ORDER.containsKey(hardDependency), "插件（%s）缺少硬依赖（%s）".formatted(identifier, hardDependency));

                int dependencyIndex = LOAD_ORDER.get(hardDependency);
                Assertions.assertTrue(index > dependencyIndex, "插件（%s）未在软依赖之前加载。（%s）".formatted(identifier, hardDependency));
            }
            ...
            for (String softDependency : pluginMeta.getPluginSoftDependencies()) {
                ...
                Assertions.assertTrue(index > dependencyIndex, "插件（%s）未在软依赖之前加载。（%s）".formatted(identifier, softDependency));
            }
            ...
            for (String loadBefore : pluginMeta.getLoadBeforePlugins()) {
                ...
                Assertions.assertTrue(index < dependencyIndex, "插件（%s）未在 loadbefore 依赖之前加载。（%s）".formatted(identifier, loadBefore));
            }
        }
    }
}
