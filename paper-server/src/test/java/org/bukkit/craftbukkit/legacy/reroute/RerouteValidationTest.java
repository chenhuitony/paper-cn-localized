    ...
    private String isValid(String key, RerouteMethodData rerouteMethodData) {
        try {
            ...
            if (method.getReturnType() != returnClazz) {
                return "返回类型不匹配，应为 " + method.getReturnType().getName() + " got: " + returnClazz;
            }
            ...
            if (Modifier.isStatic(method.getModifiers()) && !rerouteMethodData.staticReroute()) {
                return "方法是 static，但重路由数据未标记为 static 重路由";
            }

            if (!Modifier.isStatic(method.getModifiers()) && rerouteMethodData.staticReroute()) {
                return "方法不是 static，但重路由数据被标记为 static 重路由";
            }
        } catch (ClassNotFoundException e) {
            return "不存在的类：" + e.getLocalizedMessage();
        } catch (NoSuchMethodException e) {
            return "不存在的方法：" + e.getLocalizedMessage();
        }
    ...
    private Class<?> toClass(Type type) throws ClassNotFoundException {
        if (type.getSort() == Type.OBJECT) {
            ...
            return switch (type.getSort()) {
                ...
                default -> throw new UnsupportedOperationException("不支持的类型：" + type);
            };
        }
    }
}
