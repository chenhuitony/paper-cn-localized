...
public abstract class AbstractRerouteTest {
    ...
    void test(Class<?> testClass, Predicate<String> predicate, List<TestDataHolder> dataList) {
        ...
        for (TestDataHolder testHolder : dataList) {
            ...
            assertEquals(testHolder.rerouteMethodDataList().size(), holder.rerouteMethodDataMap.size(), String.format("第二层重路由数据数量不一致。应为：%s，实际：%s", testHolder.rerouteMethodDataList, holder.rerouteMethodDataMap));
            ...
            for (RerouteMethodData testData : testHolder.rerouteMethodDataList()) {
                ...
                assertNotNull(actual, String.format("未找到 %s 的重路由方法数据", testData));
    ...
    private void check(RerouteMethodData actual, RerouteMethodData expected) {
        assertEquals(expected.methodKey(), actual.methodKey(), "方法键不一致");
        assertEquals(expected.sourceDesc(), actual.sourceDesc(), "源描述不一致");
        assertEquals(expected.sourceOwner(), actual.sourceOwner(), "源所有者不一致");
        assertEquals(expected.sourceName(), actual.sourceName(), "源名称不一致");
        assertEquals(expected.staticReroute(), actual.staticReroute(), "静态重路由标志不一致");
        assertEquals(expected.targetType(), actual.targetType(), "目标类型不一致");
        assertEquals(expected.targetOwner(), actual.targetOwner(), "目标所有者不一致");
        assertEquals(expected.targetName(), actual.targetName(), "目标名称不一致");
        ...
        assertEquals(expected.rerouteReturn(), actual.rerouteReturn(), "重路由返回标志不一致");
        assertEquals(expected.isInBukkit(), actual.isInBukkit(), "In Bukkit flag are not the same");
        assertEquals(expected.requiredPluginVersion(), actual.requiredPluginVersion(), "所需插件版本不一致");
    }
