            ...
            for (AbstractInsnNode instruction : methodNode.instructions) {
                ...
                if (instruction instanceof MethodInsnNode methodInsnNode) {
                    ...
                    if (this.check(methodInsnNode.owner, methodInsnNode.name, methodInsnNode.desc)) {
                        // Add to list
                        missingReason.add(String.format("方法名：%s，名称：%s，行号：%s", methodNode.name, methodInsnNode.name, lastLineNumber.line));
                    }
                    ...
                    if (this.check(handle.getOwner(), handle.getName(), handle.getDesc())) {
                        // Add to list
                        missingReason.add(String.format("[D] 方法名：%s，名称：%s，行号：%s", methodNode.name, handle.getName(), lastLineNumber.line));
                    }
                }
            }
        }
